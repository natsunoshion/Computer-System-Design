...
static inline void rtl_sext(rtlreg_t *dest, const rtlreg_t *src1,
                            int width) { // ·ûºÅÀ©Õ¹
  // dest <- signext(src1[(width * 8 - 1) .. 0])
  rtl_shli(dest, src1, (4 - width) * 8); // ÏÈÂß¼­×óÒÆ
  rtl_sari(dest, dest, (4 - width) * 8); // ÔÙËãÊõÓÒÒÆ
}
...
static inline void rtl_msb(rtlreg_t *dest, const rtlreg_t *src1,
                           int width) { // ·µ»Ø·ûºÅÎ»
  // dest <- src1[width * 8 - 1]
