  ...
  for (int i = p; i < q; i++) {
    ...
    if (cnt <= 0) {
      if (cnt < 0) { // brankets unmatched
        ...
        return false; // not in the (<expr>) format
    }
  }
    ...
    for (int i = p; i <= q; i++) {
      ...
      // if token is operator
        ...
        if (get_operator_priority(tokens[i].type) >
            get_operator_priority(operators_stack[operators_index].type)) {
          if (tokens[i].type == '(') {
        ...
        // else calculate the tmp result
          ...
          while (operators_stack[operators_index].type != TK_NOTYPE &&
            ...
            if (operators_stack[operators_index].type == ')') {
              ...
              case '+':
                operands_stack[operands_index - 1] =
              ...
              case TK_DEREF:
                operands_stack[operands_index] =
                    paddr_read(operands_stack[operands_index], 4);
                break;
              default:
                success = false;
                return 0;
