...
make_group(gp1, EX(add), EX(or), EX(adc), EX(sbb), EX(and), EX(sub), EX(xor),
    ...
    make_group(gp2, EX(rol), EMPTY, EMPTY, EMPTY, EX(shl), EX(shr), EMPTY,
               EX(sar))
    ...
    /* 0xff */
    make_group(gp5, EX(inc), EX(dec), EX(call_rm), EMPTY, EX(jmp_rm), EMPTY,
               EX(push), EMPTY)

    /* 0x0f 0x01*/
    make_group(gp7, EMPTY, EMPTY, EMPTY, IDEX(lidt_a, lidt), EMPTY, EMPTY,
               EMPTY, EMPTY)
    ...
    opcode_entry opcode_table[512] = {
        ...
        /* 0xe4 */ IDEXW(in_I2a, in, 1), IDEXW(in_I2a, in, 1),
        IDEXW(out_a2I, out, 1), IDEXW(out_a2I, out, 1),
