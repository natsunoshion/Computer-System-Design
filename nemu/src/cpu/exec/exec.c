...
make_group(gp1, EX(add), EX(or), EX(adc), EX(sbb), EX(and), EX(sub), EX(xor),
    ...
    make_group(gp2, EMPTY, EMPTY, EMPTY, EMPTY, EXW(shl, 1), EX(shr), EMPTY,
               EXW(sar, 1))

    /* 0xf6, 0xf7 */
    make_group(gp3, IDEX(test_I, test), EMPTY, EX(not), EX(neg), EX(mul),
               EX(imul1), EX(div), EX(idiv))

    /* 0xfe */
    make_group(gp4, EX(inc), EX(dec), EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY)

    /* 0xff */
    make_group(gp5, EX(inc), EX(dec), EX(call_rm), EX(call), EX(jmp_rm), EMPTY,
               EX(push), EMPTY)
    ...
    opcode_entry opcode_table[512] = {
        ...
        /* 0x08 */ IDEXW(G2E, or, 1), IDEX(G2E, or), IDEXW(E2G, or, 1),
        IDEX(E2G, or),
        ...
        /* 0x28 */ IDEXW(G2E, sub, 1), IDEX(G2E, sub), IDEXW(E2G, sub, 1),
        IDEX(E2G, sub),
        ...
        /* 0x70 */ IDEXW(J, jcc, 1), IDEXW(J, jcc, 1), IDEXW(J, jcc, 1),
        IDEXW(J, jcc, 1),
        ...
        /* 0xec */ IDEXW(in_dx2a, in, 1), IDEX(in_dx2a, in),
        IDEXW(out_a2dx, out, 1), IDEX(out_a2dx, out),
        ...
        /* 0x20 */ IDEX(mov_load_cr, mov), EMPTY,
        IDEX(mov_store_cr, mov_store_cr), EMPTY,
        ...
        /* 0xa4 */ IDEX(Ib_G2E, shld), IDEX(Ib_G2E, shld), EMPTY, EMPTY,
        /* 0xa8 */ EMPTY, EMPTY, EMPTY, EMPTY,
        /* 0xac */ IDEX(Ib_G2E, shrd), IDEX(Ib_G2E, shrd), EMPTY,
        IDEX(E2G, imul2),
...
static inline void update_eip(void) {
  cpu.eip = (decoding.is_jmp ? (decoding.is_jmp = 0, decoding.jmp_eip)
                             : decoding.seq_eip);
}
