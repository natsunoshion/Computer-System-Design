...
--*/
{
  ...
  gpGlobals->lpObjectDesc =
      PAL_LoadObjectDesc(va("%s%s", PAL_PREFIX, "desc.dat"));
#endif
...
--*/
{
  ...
  PAL_MKFReadChunk((LPBYTE)(p->rgObject), sizeof(p->rgObject), 2,
                   gpGlobals->f.fpSSS);
...
/*++
  Purpose:
    ...
    Initialize the game data (used when starting a new game or loading a saved
game).
...
/*++
  ...
  Parameters:
    ...
    [IN]  iNum - number to be added (positive value) or removed (negative
value).
...
--*/
{
  ...
  memset(&(gpGlobals->rgEquipmentEffect), 0,
         sizeof(gpGlobals->rgEquipmentEffect));
...
--*/
{
  ...
  for (i = 0; i <= MAX_PLAYER_EQUIPMENTS; i++) {
    w += gpGlobals->rgEquipmentEffect[i]
             .rgwElementalResistance[iAttrib][wPlayerRole];
  }
...
--*/
{
  ...
  for (i = 0; i <= MAX_PLAYER_EQUIPMENTS; i++) {
    if (gpGlobals->rgEquipmentEffect[i].rgwSpriteNumInBattle[wPlayerRole] !=
        0) {
...
--*/
{
  ...
  case kStatusHaste:
    //
    // for "good" statuses, reset the status if the status to be set lasts
    // longer
    //
    if (gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] != 0 &&
        gpGlobals->rgPlayerStatus[wPlayerRole][wStatusID] < wNumRound) {
