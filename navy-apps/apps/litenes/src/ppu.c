...
static const word ppu_base_nametable_addresses[4] = {0x2000, 0x2400, 0x2800,
                                                     0x2C00};
...
void ppu_draw_background_scanline(bool mirror) {
  ...
  int taddr =
      ppu_base_nametable_address() + (tile_y << 5) + (mirror ? 0x400 : 0);
  int y_in_tile = ppu.scanline & 0x7;
  int scroll_base = -ppu.PPUSCROLL_X + (mirror ? 256 : 0);
  word attribute_address =
      (ppu_base_nametable_address() + (mirror ? 0x400 : 0) + 0x3C0 + -1 +
       ((ppu.scanline >> 5) << 3));
  ...
  for (tile_x = ppu_shows_background_in_leftmost_8px() ? 0 : 1; tile_x < 32;
    ...
    word tile_address =
        ppu_background_pattern_table_address() + (tile_index << 4);
