...
bool cpu_op_in_base_instruction_set[256]; // true if instruction is in base 6502
                                          // instruction set
...
inline byte cpu_ram_read(word address) { return CPU_RAM[address & 0x7FF]; }

void cpu_ram_write(word address, byte data) { CPU_RAM[address & 0x7FF] = data; }
...
void cpu_op_adc() {
  ...
  cpu_modify_flag(overflow_bp,
                  !!(~(cpu.A ^ op_value) & (cpu.A ^ result) & 0x80));
  cpu.A = result & 0xFF;
...
// Extended instruction set found in other CPUs and implemented for
// compatibility
...
inline unsigned long cpu_clock() { return cpu_cycles; }
